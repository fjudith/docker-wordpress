...
if [ -z create ] ; then
  ...
  kubectl apply -f ./local-volumes.yaml
  kubectl create secret generic wp-mysql-pass --from-file=password.txt
  curl ./mysql-deployment.yaml | conduit inject --skip-inbound-ports=3306 - | kubectl apply -f -
  curl ./wordpress-deployment.yaml | conduit inject --skip-outbound-ports=3306,9000,11211 --skip-inbound-ports=3306,9000,11211 - | kubectl apply -f -
